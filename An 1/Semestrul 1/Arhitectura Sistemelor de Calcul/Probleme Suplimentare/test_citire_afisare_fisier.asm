...
segment data use32 class=data
    ...
    format db "Am citit %d caractere din fisier.", 0  ; formatul - utilizat pentru afisarea textului citit din fisier
...
segment code use32 class=code
    ...
    mov eax, 0
    mov ecx, 0  ; Contor pentru șirul text
    mov edx, 0  ; Contor pentru șirul caractere_speciale

    ; Parcurgere șir text și comparare cu șirul caractere_speciale
    parcurge_sir:
        mov al, [text + ecx]
        mov bl, [caractere_speciale + edx]
        cmp al, bl
        je elemente_egale

        ; Avansare la următorul element în șirul text
        inc ecx
        jmp verificare_sfarsit

    elemente_egale:
        ; Incrementare contor de elemente egale
        inc eax

    verificare_sfarsit:
        ; Verificare dacă am ajuns la sfârșitul șirului text
        cmp al, 0
        je sfarsit

        ; Verificare dacă am ajuns la sfârșitul șirului caractere_speciale
        cmp bl, 0
        je sfarsit

        ; Avansare la următorul element în șirul caractere_speciale
        inc edx
        jmp parcurge_sir
